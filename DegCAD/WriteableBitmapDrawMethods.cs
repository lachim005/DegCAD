...
namespace DegCAD
{
    public static class WriteableBitmapDrawMethods
    {
        ...
        #region Thick circle
        public static void DrawCircleThick(this WriteableBitmap bitmap, Vector2 center, int radius, Color color, int thickness = 2)
        {
            // Initialize the variables for the Bresenham's algorithm
            int x = radius;
            int y = 0;
            ...
            unsafe void SetPixel(int x, int y, int t)
            {
                if (x < 0 || y < 0 || x >= bitmap.Width || y >= bitmap.Height)
                    return;
                context.Pixels[x + context.Width * y] = colorNumber;
            ...
            while (x >= y)
            {
                SetPixel((int)center.X + x, (int)center.Y + y, thickness);
                SetPixel((int)center.X - x, (int)center.Y + y, thickness);
                SetPixel((int)center.X + x, (int)center.Y - y, thickness);
                SetPixel((int)center.X - x, (int)center.Y - y, thickness);
                SetPixel((int)center.X + y, (int)center.Y + x, thickness);
                SetPixel((int)center.X - y, (int)center.Y + x, thickness);
                SetPixel((int)center.X + y, (int)center.Y - x, thickness);
                SetPixel((int)center.X - y, (int)center.Y - x, thickness);
        ...
        public static void DrawCircleDashedThick(this WriteableBitmap bitmap, Vector2 center, int radius, Color color, int thickness = 2, int dashLength = 10)
        {
            // Initialize the variables for the Bresenham's algorithm
            int x = radius;
            int y = 0;
            ...
            unsafe void SetPixel(int x, int y, bool draw, int t)
            {
                if (x < 0 || y < 0 || x >= bitmap.Width || y >= bitmap.Height)
                    return;
                context.Pixels[x + context.Width * y] = draw ? colorNumber : 0;
                if (--t > 0 && draw)
                {
                    SetPixel(x + 1, y, draw, t);
                    SetPixel(x - 1, y, draw, t);
                    SetPixel(x, y + 1, draw, t);
                    SetPixel(x, y - 1, draw, t);
                }
            }
            ...
            while (x >= y)
            {
                ...
                SetPixel((int)center.X + x, (int)center.Y + y, draw, thickness);
                SetPixel((int)center.X - x, (int)center.Y + y, draw, thickness);
                SetPixel((int)center.X + x, (int)center.Y - y, draw, thickness);
                SetPixel((int)center.X - x, (int)center.Y - y, draw, thickness);
                SetPixel((int)center.X + y, (int)center.Y + x, draw, thickness);
                SetPixel((int)center.X - y, (int)center.Y + x, draw, thickness);
                SetPixel((int)center.X + y, (int)center.Y - x, draw, thickness);
                SetPixel((int)center.X - y, (int)center.Y - x, draw, thickness);
        ...
        public static void DrawCircleDotDashThick(this WriteableBitmap bitmap, Vector2 center, int radius, Color color, int thickness = 2, int dashLength = 10)
        {
            // Initialize the variables for the Bresenham's algorithm
            int x = radius;
            int y = 0;
            ...
            while (x >= y)
            {
                ...
                bool shouldDraw = draw || dashCounter == dashLength / 2;
                SetPixel((int)center.X + x, (int)center.Y + y, shouldDraw, thickness);
                SetPixel((int)center.X - x, (int)center.Y + y, shouldDraw, thickness);
                SetPixel((int)center.X + x, (int)center.Y - y, shouldDraw, thickness);
                SetPixel((int)center.X - x, (int)center.Y - y, shouldDraw, thickness);
                SetPixel((int)center.X + y, (int)center.Y + x, shouldDraw, thickness);
                SetPixel((int)center.X - y, (int)center.Y + x, shouldDraw, thickness);
                SetPixel((int)center.X + y, (int)center.Y - x, shouldDraw, thickness);
                SetPixel((int)center.X - y, (int)center.Y - x, shouldDraw, thickness);
